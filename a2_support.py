...
class UnoGame:
    ...
    def select_card(self, player, card):
        ...
        if card.__class__ in SPECIAL_CARDS:
            self.special_pile.add_cards(card)
        else:
            self.putdown_pile.add_cards(card)
    ...
    def get_pickup_pile(self):
        return self.pickup_pile


def build_deck(structure):
    """
    ...
    skip cards with the numbers 3 and 4.
    ...
    for (card, (start, end)) in structure:
        for number in range(start, end):
            new_card = card.__class__(card.get_number(), card.get_colour())
